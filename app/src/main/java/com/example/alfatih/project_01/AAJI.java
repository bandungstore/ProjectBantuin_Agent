...
public class AAJI extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_aaji);
        setTitle("AAJI");
        ...
        try {
            ...
            arrayBiodata = new JSONArray(data.getAllDataRekrut(wp.getId()));
            ...
            for (int i = 0; i < arrayBiodata.length(); i++) {
                ...
                calon_nasabah[i] = jsonChildNode.optString("Calon_Agen");
    ...
    public void selesaiOnClick(View view) {
        ...
        if( spinner.getSelectedItem().toString().trim().equals("")){
            Toast.makeText(getBaseContext(),"Calon agen kosong", Toast.LENGTH_LONG).show();
        }else {
            ...
            String laporan = data.insertAAJI(wp.getId(), wp.getNama(), Nama.getText().toString(), thisDate, dayOfTheWeek);
